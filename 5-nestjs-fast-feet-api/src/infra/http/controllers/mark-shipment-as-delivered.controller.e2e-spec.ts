...
describe('mark shipment as delivered (E2E)', () => {
  ...
  test('[PATCH] /shipments/:id/mark-as-delivered', async () => {
    ...
    const response = await request(app.getHttpServer())
      .patch(`/shipments/${shipment.id.toString()}/mark-as-delivered`)
    ...
    const attachmentOnDatabase = await prisma.attachment.findMany({
      where: {
        shipmentId: shipmentOnDatabase?.id,
      },
    })
