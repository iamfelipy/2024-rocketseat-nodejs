...
interface EditRecipientByRecipientUseCaseRequest {
  recipientId: string
  cpf: string
  password: string
  roles: string[]
...
type EditRecipientByRecipientUseCaseResponse = Either<NotAuthorizedError | ResourceNotFoundError, {
  recipient: Recipient
}>

export class EditRecipientByRecipientUseCase {
  ...
  async execute({
    ...
    latitude,
    longitude,
  }:EditRecipientByRecipientUseCaseRequest): Promise<EditRecipientByRecipientUseCaseResponse> {
