...
type MarkShipmentAsDeliveredUseCaseResponse = Either<ResourceNotFoundError | NotAllowedError, {
  shipment: Shipment
}>
...
export class MarkShipmentAsDeliveredUseCase {
  ...
  async execute({
    ...
    const shipmentAttachmentList = new ShipmentAttachmentList(shipmentAttachments)

    if (!shipmentAttachmentList || shipmentAttachmentList?.currentItems?.length === 0) {
